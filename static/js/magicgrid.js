...
// 각 이미지 요소마다 반복하여 실행한다.
for (let i = 0; i < masonrys.length; i++) {
  // 이미지가 로드되었을 때의 이벤트 리스너를 추가한다.
  masonrys[i].addEventListener('load', function() {
    // magicGrid라는 객체의 positionItems() 메서드를 호출하여
    // 이미지들을 적절한 위치에 배치한다.
    magicGrid.positionItems();
  }, false);
}
/*
HTML 문서 내에서 <img> 요소들을 가져와서, 각 이미지가 로드될 때마다
magicGrid 객체의 positionItems() 메서드를 호출하여 이미지들을 적절한 위치에 배치하는 것을 목적
 */
